...
const NAV_INSPECTOR=[['inicio','▣','Inicio'],['comercios','🏪','Comercios'],['inspecciones','📋','Inspecciones'],['normativa','📚','Normativa HST'],['reportes','📊','Reportes']];
...
function navIcon(id){
  const icons={
    ...
    reportes:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 21V10"/><path d="M12 21V4"/><path d="M19 21v-7"/><path d="M3 21h18"/></svg>',normativa:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 5.5A3.5 3.5 0 0 1 7.5 2H11v18H7.5A3.5 3.5 0 0 0 4 23z"/><path d="M20 5.5A3.5 3.5 0 0 0 16.5 2H13v18h3.5A3.5 3.5 0 0 1 20 23z"/></svg>'
  };
  return icons[id]||'';
}
function renderApp(){
...
async function renderView(view){const c=document.getElementById('content');c.innerHTML='<div class="loading">Cargando información…</div>';try{if(view==='inicio')return renderDashboard(c);if(view==='comercios')return renderCommerces(c);if(view==='inspecciones')return renderInspections(c);if(view==='normativa')return renderNormativas(c);if(view==='usuarios')return renderUsers(c);if(view==='reportes')return renderReports(c);if(view==='obras')return renderSimpleTable(c,'Obras privadas','works','responsible,address,status,created_at');if(view==='contribuyentes')return renderSimpleTable(c,'Contribuyentes','contributors','name,dni_cuit,activity,status');if(view==='infracciones')return renderSimpleTable(c,'Infracciones','infractions','code,reason,status,infraction_date')}catch(e){console.error(e);c.innerHTML='<div class="panel"><div class="form-error">No se pudo cargar este módulo.</div></div>'}}
...
function inspectionTable(rows){
  ...
  return `<div class="table-wrap"><table class="data-table"><thead><tr><th>Fecha</th><th>Comercio</th><th>Inspector</th><th>Expediente / Acta</th><th>Estado</th>${admin?'<th>Mapa</th>':''}</tr></thead><tbody>${rows.map(r=>{
    const hasGps=Number.isFinite(Number(r.latitude))&&Number.isFinite(Number(r.longitude));
    const mapCell=admin
      ?(hasGps
        ?`<button class="btn map-inspection-btn" type="button" data-map-inspection="${esc(r.id)}">📍 Ver en mapa</button>`
        :'<span class="muted">Sin GPS</span>')
      :'';
    return `<tr><td>${fmtDate(r.inspected_at)}</td><td>${esc(r.commerces?.business_name||'—')}</td><td>${esc(r.profiles?.full_name||'—')}</td><td>${esc(r.act_number||'—')}</td><td><span class="badge ${r.status==='realizada'?'success':r.status==='observada'?'warning':''}">${esc(r.status||'—')}</span></td>${admin?`<td>${mapCell}</td>`:''}</tr>`;
  }).join('')}</tbody></table></div>`;
}
async function renderCommerces(c){
  const {data,error}=await sb.from('commerces')
    .select('id,business_name,legal_name,cuit,commercial_license,address,sector,zone,status,inspection_state,last_inspected_at,last_inspected_by,last_inspected_by_name,last_inspection_id')
    .order('business_name').limit(1000);
  if(error)throw error;
  commercesCache=data||[];
  c.innerHTML=`<div class="page-head"><div><h1>Comercios</h1><p>Base municipal unificada de establecimientos.</p></div><div class="toolbar"><input class="searchbox" id="commerceSearch" placeholder="Buscar comercio, dirección o CUIT"></div></div><div class="panel" id="commerceTable"></div>`;

  const render=rows=>{
    document.getElementById('commerceTable').innerHTML=`<h3>${rows.length} comercios</h3><div class="table-wrap"><table class="data-table"><thead><tr><th>Comercio</th><th>CUIT</th><th>Habilitación</th><th>Dirección</th><th>Sector</th><th>Inspección</th>${currentProfile.role==='inspector'?'<th>Acción</th>':''}</tr></thead><tbody>${rows.map(r=>{
      const inspected=r.inspection_state==='inspeccionado'&&r.last_inspected_at;
      const inspectionCell=inspected
        ?(currentProfile.role==='admin'
          ?`<span class="badge success">Inspeccionado</span><div class="inspection-meta">Por <strong>${esc(r.last_inspected_by_name||'Inspector')}</strong><br>${esc(fmtDate(r.last_inspected_at))}</div>`
          :'<span class="badge success">Ya inspeccionado</span>')
        :'<span class="badge warning">Pendiente</span><div class="inspection-meta">Sin inspección registrada</div>';
      const action=currentProfile.role==='inspector'
        ?(inspected
          ?'<button class="btn commerce-inspected-btn" type="button" disabled>✓ Ya inspeccionado</button>'
          :`<button class="btn primary commerce-inspect-btn" type="button" data-inspect-commerce="${esc(r.id)}">Inspeccionar</button>`)
        :'';
      return `<tr><td><strong>${esc(r.business_name)}</strong><br><small class="muted">${esc(r.legal_name||'')}</small></td><td>${esc(r.cuit||'—')}</td><td>${esc(r.commercial_license||'—')}</td><td>${esc(r.address||'—')}</td><td>${esc(r.sector||'—')}</td><td>${inspectionCell}</td>${currentProfile.role==='inspector'?`<td>${action}</td>`:''}</tr>`;
    }).join('')}</tbody></table></div>`;
    document.querySelectorAll('[data-inspect-commerce]').forEach(btn=>btn.onclick=()=>openNewInspectionModal(btn.dataset.inspectCommerce,'comercios'));
  };

  render(commercesCache);
  document.getElementById('commerceSearch').oninput=e=>{
    const q=e.target.value.toLowerCase().trim();
    render(commercesCache.filter(r=>[r.business_name,r.legal_name,r.cuit,r.address,r.sector,r.zone,r.last_inspected_by_name,r.inspection_state].some(v=>String(v||'').toLowerCase().includes(q))));
  };
}
async function renderInspections(c){
  const {data,error}=await sb.from('inspections')
    .select('id,inspected_at,status,act_number,observations,qr_scanned,latitude,longitude,gps_accuracy_m,commerces(business_name,address),profiles(full_name)')
    .order('inspected_at',{ascending:false}).limit(500);
  if(error)throw error;
  ...
  if(currentProfile.role==='admin'){
    document.querySelectorAll('[data-map-inspection]').forEach(btn=>{
      btn.onclick=()=>{
        const row=(data||[]).find(x=>x.id===btn.dataset.mapInspection);
        if(row)openInspectionMap(row);
      };
    });
  }
}

function openInspectionMap(r){
  if(currentProfile.role!=='admin')return;
  const lat=Number(r.latitude),lon=Number(r.longitude);
  if(!Number.isFinite(lat)||!Number.isFinite(lon)){toast('Esta inspección no tiene ubicación GPS registrada.');return}
  const label=`${r.commerces?.business_name||'Inspección'}`;
  const embedUrl=`https://maps.google.com/maps?q=${encodeURIComponent(lat+','+lon)}&z=17&output=embed`;
  const mapsUrl=`https://www.google.com/maps?q=${encodeURIComponent(lat+','+lon)}`;
  const modal=document.createElement('div');
  modal.className='modal-backdrop';
  modal.innerHTML=`<div class="modal-card map-modal-card">
    <div class="modal-head">
      <div><h2>Ubicación de la inspección</h2><p><strong>${esc(label)}</strong> · ${esc(fmtDate(r.inspected_at))}</p></div>
      <button class="modal-close" type="button" aria-label="Cerrar">×</button>
    </div>
    <div class="inspection-map-meta">
      <div><span>Inspector</span><strong>${esc(r.profiles?.full_name||'—')}</strong></div>
      <div><span>Dirección</span><strong>${esc(r.commerces?.address||'—')}</strong></div>
      <div><span>Coordenadas</span><strong>${lat.toFixed(6)}, ${lon.toFixed(6)}</strong></div>
      <div><span>Precisión GPS</span><strong>${r.gps_accuracy_m?('±'+Math.round(Number(r.gps_accuracy_m))+' m'):'No informada'}</strong></div>
    </div>
    <div class="inspection-map-frame"><iframe title="Ubicación en Google Maps" src="${embedUrl}" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe></div>
    <div class="modal-actions"><a class="btn primary" href="${mapsUrl}" target="_blank" rel="noopener">Abrir en Google Maps ↗</a><button type="button" class="btn" id="closeMap">Cerrar</button></div>
  </div>`;
  document.body.appendChild(modal);
  const close=()=>modal.remove();
  modal.querySelector('.modal-close').onclick=close;
  modal.querySelector('#closeMap').onclick=close;
  modal.addEventListener('click',e=>{if(e.target===modal)close()});
}
async function openNewInspectionModal(preselectedCommerceId='',returnView='inspecciones'){
  let commerces=commercesCache;
  if(!commerces.length||!Object.prototype.hasOwnProperty.call(commerces[0]||{},'inspection_state')){
    const {data,error}=await sb.from('commerces')
      .select('id,business_name,address,status,inspection_state,last_inspected_at,last_inspected_by_name')
      .eq('active',true).order('business_name').limit(1000);
    if(error){toast('No se pudo cargar la lista de comercios.');return}
    commerces=data||[];
  }

  const selectedCommerce=preselectedCommerceId?commerces.find(x=>x.id===preselectedCommerceId):null;
  if(currentProfile.role==='inspector'&&selectedCommerce?.inspection_state==='inspeccionado'){
    toast(`Este comercio ya fue inspeccionado por ${selectedCommerce.last_inspected_by_name||'otro inspector'}.`);
    return;
  }

  const selectable=currentProfile.role==='inspector'
    ?commerces.filter(x=>x.inspection_state!=='inspeccionado')
    :commerces;
  ...
  modal.innerHTML=`<div class="modal-card">
    ...
    <form id="inspectionForm">
      <div class="form-grid">
        <div class="field span-2"><label>Comercio *</label><select id="inspectionCommerce" required><option value="">Seleccionar comercio…</option>${selectable.map(x=>`<option value="${esc(x.id)}">${esc(x.business_name)} — ${esc(x.address||'Sin dirección')}</option>`).join('')}</select><div class="field-help">Los comercios ya inspeccionados no aparecen disponibles para un nuevo inspector.</div></div>
  ...
  document.body.appendChild(modal);
  const commerceSelect=modal.querySelector('#inspectionCommerce');
  if(preselectedCommerceId&&selectable.some(x=>x.id===preselectedCommerceId)){
    commerceSelect.value=preselectedCommerceId;
  ...
  modal.querySelector('#inspectionForm').onsubmit=e=>saveInspection(e,modal,gps,close,returnView);
}
...
async function saveInspection(e,modal,gps,close,returnView='inspecciones'){
  e.preventDefault();
  ...
  try{
    if(currentProfile.role==='inspector'){
      const {data:commerceState,error:stateError}=await sb.from('commerces')
        .select('inspection_state,last_inspected_at,last_inspected_by_name')
        .eq('id',commerceId).single();
      if(stateError)throw stateError;
      if(commerceState?.inspection_state==='inspeccionado'){
        errorBox.innerHTML=`<div class="form-error">Este comercio ya fue inspeccionado por <strong>${esc(commerceState.last_inspected_by_name||'otro inspector')}</strong> el ${esc(fmtDate(commerceState.last_inspected_at))}. No se guardó una inspección duplicada.</div>`;
    ...
    close();
    toast(photoErrors.length?'Inspección guardada. Algunas fotos no pudieron subirse.':'Comercio marcado como inspeccionado correctamente.');
...
async function renderNormativas(c){
  if(currentProfile.role!=='inspector'){
    c.innerHTML='<div class="panel"><div class="form-error">Este módulo está disponible para usuarios Inspectores.</div></div>';
    return;
  }
  const {data,error}=await sb.from('normative_catalog')
    .select('id,code,title,category,section,scope_topic,regulation_detail,practical_use,status_note,source_url,sort_order')
    .eq('active',true)
    .not('section','is',null)
    .order('sort_order',{ascending:true});
  if(error)throw error;
  const rows=data||[];
  c.innerHTML=`<div class="page-head"><div><h1>Normativa HST</h1><p>Compendio práctico para consulta durante inspecciones.</p></div></div>
    <div class="normative-intro">
      <div class="normative-intro-icon">§</div>
      <div><strong>Normativa argentina de Higiene y Seguridad en el Trabajo</strong><p>Seleccioná una normativa para consultar su alcance, qué regula y su uso práctico. Verificá siempre el texto oficial vigente antes de emitir un informe profesional.</p></div>
    </div>
    <div class="normative-tools"><input id="normSearch" class="searchbox" placeholder="Buscar ley, decreto, resolución o tema…"><select id="normSection"><option value="">Todas las secciones</option>${[...new Set(rows.map(r=>r.section).filter(Boolean))].map(s=>`<option value="${esc(s)}">${esc(s)}</option>`).join('')}</select></div>
    <div id="normativeList"></div>`;

  const list=document.getElementById('normativeList');
  const search=document.getElementById('normSearch');
  const section=document.getElementById('normSection');
  const draw=()=>{
    const q=search.value.toLowerCase().trim(), sec=section.value;
    const filtered=rows.filter(r=>{
      const matchesText=[r.code,r.title,r.category,r.section,r.scope_topic,r.regulation_detail,r.practical_use].some(v=>String(v||'').toLowerCase().includes(q));
      return matchesText&&(!sec||r.section===sec);
    });
    list.innerHTML=filtered.length?`<div class="normative-list">${filtered.map(r=>`<button class="normative-item" data-norm-id="${r.id}">
      <div class="normative-item-code">${esc(r.code)}</div>
      <div class="normative-item-body"><strong>${esc(r.title)}</strong><span>${esc(r.section||r.category||'Normativa HST')}</span></div>
      <div class="normative-item-arrow">›</div>
    </button>`).join('')}</div>`:'<div class="panel empty">No se encontraron normativas con ese criterio.</div>';
    list.querySelectorAll('[data-norm-id]').forEach(btn=>btn.onclick=()=>openNormativeDetail(c,rows.find(r=>r.id===btn.dataset.normId)));
  };
  search.oninput=draw;section.onchange=draw;draw();
}
function openNormativeDetail(c,r){
  if(!r)return;
  c.innerHTML=`<div class="normative-detail-head"><button class="btn norm-back" id="backNorm">← Volver a normativas</button></div>
    <article class="normative-detail-card">
      <div class="normative-detail-top">
        <div><span class="normative-section-chip">${esc(r.section||'Normativa HST')}</span><h1>${esc(r.code)}</h1><h2>${esc(r.title)}</h2></div>
      </div>
      <div class="normative-detail-grid">
        <section><span>Ámbito / tema</span><p>${esc(r.scope_topic||'No especificado en el compendio.')}</p></section>
        <section><span>Qué regula</span><p>${esc(r.regulation_detail||'No especificado en el compendio.')}</p></section>
        <section><span>Uso práctico</span><p>${esc(r.practical_use||'No especificado en el compendio.')}</p></section>
        <section><span>Estado orientativo</span><p>${esc(r.status_note||'Consultar texto actualizado.')}</p></section>
      </div>
      <div class="normative-source-note"><strong>Fuente indicada en el compendio:</strong> Argentina.gob.ar – Normativa Nacional.<br><span>Esta ficha es una guía de consulta y no sustituye la verificación del texto oficial vigente.</span></div>
    </article>`;
  document.getElementById('backNorm').onclick=()=>renderNormativas(c);
}
